...
var BASEURL = ""; //backlog開いたときのurlのbase部分。{teamName}に値入れてください
var slackMemberSheetId = ""; //slackのidとbacklogのidを変換するのに使ってるスプレッドシートのid
var colSlackMemberId = 0;
var colBacklogMemberId = 6;
var alertMessage = "「Backlogが更新されました」"; //botからのメッセージ
//スプレッドシート取得
var book = SpreadsheetApp.openByUrl("");
//シート読み取り
var sheet = book.getSheetByName("test"); // シートネームを test にしています
...
function makeChatMessage(body) {
  ...
  switch (body.type) {
    case 1:
      ...
      bl_url =
        BASEURL + "view/" + body.project.projectKey + "-" + body.content.key_id;
        ...
        for (var c = 0; c < body.content.changes.length; c++) {
          switch (body.content.changes[c].field) {
            case "status":
              bl_changes +=
                "・状態を「" +
                backlog_changes_status[body.content.changes[c].old_value] +
                "」から「" +
                backlog_changes_status[body.content.changes[c].new_value] +
                "」に変更しました。\n";
              break;
            case "priority":
              bl_changes +=
                "・優先度を「" +
                backlog_changes_priority[body.content.changes[c].old_value] +
                "」から「" +
                backlog_changes_priority[body.content.changes[c].new_value] +
                "」に変更しました。\n";
              break;
            case "assigner":
              bl_changes +=
                "・担当者を" +
                body.content.changes[c].old_value +
                "から" +
                body.content.changes[c].new_value +
                "に変更しました。\n";
              break;
            case "component":
              bl_changes +=
            ...
            case "description":
              ...
              var stringChangeLength =
                body.content.changes[c].new_value.length -
                body.content.changes[c].old_value.length;
            ...
            case "resolution":
              bl_changes +=
                "・完了理由を「" +
                backlog_changes_resolution[body.content.changes[c].old_value] +
                "」から「" +
                backlog_changes_resolution[body.content.changes[c].new_value] +
                "」に変更しました。\n";
              break;
            case "version":
              bl_changes +=
            ...
            case "attachment":
              bl_changes +=
                body.content.changes[c].new_value + "を添付しました。\n";
              break;
            case "estimatedHours":
              bl_changes +=
                "・予定時間を「" +
    ...
    case 11:
      ...
      bl_summary = "";
      bl_url =
        BASEURL + "rev/" + body.project.projectKey + "/" + body.content.rev;
    ...
    case 12:
      ...
      bl_url =
        ...
        "/" +
        body.content.revision_type +
        "/" +
        body.content.revisions[0].rev;
...
function get_mention(text) {
  ...
  for (var i in slackMemberArray) {
    if (slackMemberArray[i][colBacklogMemberId] == "") {
      ...
      var backlogId = new RegExp(
        "@" + slackMemberArray[i][colBacklogMemberId],
        "g"
      );
      text = text.replace(
        backlogId,
        "<@" + slackMemberArray[i][colSlackMemberId] + "> "
      );
    }
  }
  return text;
...
function doGet(e) {
  ...
  if (e.postData.contents) {
    ...
    sheet.appendRow([
      new Date(),
      get_mention(slackObj["message"]),
      get_mention(slackObj["changes"])
    ]);
    // Slack投稿
    if (slackObj) {
      postSlack(
        ...
        get_mention(slackObj["message"]),
        get_mention(slackObj["changes"]),
        get_mention(slackObj["description"]),
        get_mention(slackObj["comment"])
      );
    }
  } else {
